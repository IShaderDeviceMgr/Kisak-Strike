...
| `CheckStuck`, `FixPlayerCrouchStuck`, `IsMovingPlayerStuck`, `UnblockPusher` | The unstick passes — they nudge a player out of geometry they should never have been in. The path into that state now exists (`src/server/push.rs`: a door closing on you, a platform rising through you), which promoted the first three from "unreachable" to "not written". **`UnblockPusher` is different, and it is a measurement**: `CGameMovement::UnblockPusher` (`gamemovement.cpp:3752`) has an **empty body** in the shipped tree, and `SpeculativelyCheckPush` calls it and then tests whether the player's origin changed — so in the shipped game a shoved player who ends up stuck *always* blocks the mover. The port reproduces that; see `rustdocs/SERVER.md` gotcha 86. |
